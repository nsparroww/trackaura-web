...
/* ──────────────────────────────────────────────────────────────────────
   ...
   Without `generateStaticParams`, a dynamic-segment route in Next.js 16
   is fully dynamic by default — `export const revalidate = N` is
   silently ignored. The Next.js docs are explicit on this:

     "You must return an empty array from generateStaticParams or
      utilize export const dynamic = 'force-static' in order to
      revalidate (ISR) paths at runtime."
     — https://nextjs.org/docs/app/api-reference/functions/generate-static-params

   Returning [] means: don't pre-render any paths at build time. First
   request to /p/<slug> generates and caches via ISR; subsequent
   requests within the revalidate window serve from the Vercel edge
   cache. After the window, the next request triggers stale-while-
   revalidate regeneration.

   `dynamicParams = true` is the default but stated explicitly here so
   the pattern transfers cleanly to the other catalog routes
   (/chip/[slug], /board/[slug], /cpu/[slug], /c/[slug], etc.) that
   need the same fix.

   Necessary co-requisite: every Supabase callsite in the render
   graph must use createCatalogClient() (cookie-free). cookies() in
   any rendered code path would force dynamic regardless of this
   export. See src/lib/queries/enrichment.ts and product.ts.
   ────────────────────────────────────────────────────────────────────── */
